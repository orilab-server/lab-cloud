'use client';

import { parseFileSizeStr } from '@/app/_shared/utils/size';
import { extractDateInStr } from '@/app/_shared/utils/slice';
import { format } from 'date-fns';
import React from 'react';
import { AiFillFile, AiFillFolder } from 'react-icons/ai';
import { TrashItem } from '../../home/_types/trash';
import { useCtxMenu } from '../_hooks/useCtxMenu';
import { useRemoveFromTrash } from '../_hooks/useRemoveFromTrash';
import { useRestoreFromTrash } from '../_hooks/useRestoreFromTrash';
import { useTrashSelect } from '../_hooks/useTrashSelect';

type Props = {
  trashItems: TrashItem[];
};

const TrashList = ({ trashItems }: Props) => {
  const { selected, add, onClickWithKey } = useTrashSelect(trashItems);
  const { ctxMenuRef, showCtxMenu, setShowCtxMenu, onCtxMenu } = useCtxMenu();
  const removeItemsMutation = useRemoveFromTrash();
  const restoreItemsMutation = useRestoreFromTrash();

  const removeItems = () => {
    if (confirm('削除しますか？')) {
      const formData = new FormData();
      formData.append(
        'ids',
        [...Array.from(new Set([...Array.from(selected), showCtxMenu]))].join(','),
      );
      removeItemsMutation.mutate({ formData });
      setShowCtxMenu(null);
    }
  };

  const restoreItems = () => {
    const formData = new FormData();
    formData.append(
      'ids',
      [...Array.from(new Set([...Array.from(selected), showCtxMenu]))].join(','),
    );
    restoreItemsMutation.mutate({ formData });
    setShowCtxMenu(null);
  };

  return (
    <>
      <div className="grid grid-cols-6 w-[calc(100vw_-_16rem)] min-w-[800px] px-2 py-1 divide-x border-b fixed top-14 bg-white">
        <div className="col-span-3 pl-2">名前</div>
        <div className="pl-2">サイズ</div>
        <div className="pl-2">種類</div>
        <div className="pl-2">追加日</div>
      </div>
      <div className="pt-9"></div>
      {trashItems.map((item, i) => {
        const [fileName] = extractDateInStr(item.name);
        return (
          <div
            id={item.id}
            key={item.id}
            onClick={(e) => onClickWithKey(e, item.id)}
            onContextMenu={(e) => {
              onCtxMenu(e, item.id);
              add(item.id);
            }}
            className={`relative grid grid-cols-6 px-2 mx-2 py-1 rounded-md ${
              selected.has(item.id) ? 'bg-blue-300' : (i + 1) % 2 ? '' : 'bg-gray-200'
            } cursor-pointer`}
            ref={ctxMenuRef}
          >
            <div className="col-span-3 flex items-center">
              {item.type === 'dir' ? (
                <AiFillFolder className="mr-2 text-gray-600" />
              ) : (
                <AiFillFile className="mr-2 text-gray-600" />
              )}
              <span className="">{fileName}</span>
            </div>
            <div className="pl-3 text-sm">{parseFileSizeStr(item.size)}</div>
            <div className="pl-3 text-sm">
              {item.type === 'dir'
                ? 'フォルダ'
                : `${item.name.slice(item.name.lastIndexOf('.') + 1).toUpperCase()}ファイル`}
            </div>
            <div className="pl-3 text-sm">
              {format(new Date(item.created_at), 'yyyy/MM/dd hh:mm')}
            </div>
            {/* context menu */}
            <div
              className={`${
                showCtxMenu && showCtxMenu === item.id ? '' : 'hidden'
              } absolute top-3 right-[750px] bg-gray-800 rounded z-10 text-sm`}
            >
              <div className="mx-auto rounded border border-gray-600 py-3 grid grid-cols-1 gap-2 whitespace-nowrap">
                <button onClick={removeItems} className="text-white hover:bg-gray-700 px-3 py-1">
                  ゴミ箱から削除
                </button>
                <button onClick={restoreItems} className="text-white hover:bg-gray-700 px-3 py-1">
                  元の場所に戻す
                </button>
              </div>
            </div>
          </div>
        );
      })}
      <div className="pb-9"></div>
      <div className="fixed bottom-0 w-full pl-2 py-1 bg-white border-t flex space-x-2 text-gray-500">
        <span>Trash</span>
      </div>
    </>
  );
};

export default React.memo(TrashList);
