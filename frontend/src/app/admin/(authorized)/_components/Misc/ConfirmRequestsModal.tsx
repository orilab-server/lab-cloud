...
import { useAcceptRegisterRequests } from '@/app/admin/(authorized)/_actions/acceptRegisterRequests';
import { format } from 'date-fns';
import React from 'react';
import { Controller, useForm } from 'react-hook-form';
import { RegisterRequest } from '../../_types/register';

type ConfirmregisterRequestsModalProps = {
  registerRequests: RegisterRequest[];
  isOpen: boolean;
  close: () => void;
};

interface ConfirmregisterRequestsModalInput {
  accepts: string;
}

const ConfirmregisterRequestsModal = ({
  registerRequests,
  isOpen,
  close,
}: ConfirmregisterRequestsModalProps) => {
  const acceptRequestMutation = useAcceptRegisterRequests();

  const { control, getValues, setValue } = useForm<ConfirmregisterRequestsModalInput>({
    defaultValues: {
      accepts: '',
    },
  });
  ...
  const handleCheckAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      const all = registerRequests.map((request) => request.email).join(',');
...
export default React.memo(ConfirmregisterRequestsModal);
