...
var UserTableColumns = struct {
	...
	Grade:       "users.grade",
	IsTemporary: "users.is_temporary",
...
var UserWhere = struct {
	...
	Grade:       whereHelperint{field: "`users`.`grade`"},
	IsTemporary: whereHelperbool{field: "`users`.`is_temporary`"},
...
func (r *userR) GetFilesTrashes() FilesTrashSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetReviewers() ReviewerSlice {
	if r == nil {
		return nil
	}
	return r.Reviewers
...
var (
	userAllColumns            = []string{"id", "name", "email", "password", "grade", "is_temporary", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"name", "email", "password", "grade", "is_temporary"}
...
// FilesTrashes retrieves all the files_trash's FilesTrashes with an executor.
func (o *User) FilesTrashes(mods ...qm.QueryMod) filesTrashQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`files_trash`.`user_id`=?", o.ID),
	)
...
func (o *User) Revieweds(mods ...qm.QueryMod) reviewedQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`reviewed`.`user_id`=?", o.ID),
	)
...
func (o *User) Reviewers(mods ...qm.QueryMod) reviewerQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// LoadFilesTrashes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFilesTrashes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`files_trash`),
		qm.WhereIn(`files_trash.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load files_trash")
	}

	var resultSlice []*FilesTrash
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice files_trash")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on files_trash")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for files_trash")
	}
	...
	if singular {
		object.R.FilesTrashes = resultSlice
...
// LoadRevieweds allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRevieweds(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`reviewed`),
		qm.WhereIn(`reviewed.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on reviewed")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.Revieweds = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &reviewedR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Revieweds = append(local.R.Revieweds, foreign)
				if foreign.R == nil {
					foreign.R = &reviewedR{}
				}
				foreign.R.User = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadReviewers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`reviewers`),
		qm.WhereIn(`reviewers.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddFilesTrashes adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.FilesTrashes.
// Sets related.R.User appropriately.
func (o *User) AddFilesTrashes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*FilesTrash) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			FilesTrashes: related,
		}
	} else {
		o.R.FilesTrashes = append(o.R.FilesTrashes, related...)
	}
...
// AddRevieweds adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Revieweds.
// Sets related.R.User appropriately.
func (o *User) AddRevieweds(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Reviewed) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Revieweds = append(o.R.Revieweds, related...)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddReviewers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Reviewer) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
