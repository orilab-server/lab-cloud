...
var ResetTokenTableColumns = struct {
	ID    string
	Email string
	Token string
}{
	ID:    "reset_tokens.id",
	Email: "reset_tokens.email",
	Token: "reset_tokens.token",
}
...
var (
	...
	resetTokenColumnsWithoutDefault = []string{"id", "email", "token"}
	resetTokenColumnsWithDefault    = []string{}
	resetTokenPrimaryKeyColumns     = []string{"id"}
	resetTokenGeneratedColumns      = []string{}
)
...
func ResetTokens(mods ...qm.QueryMod) resetTokenQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`reset_tokens`.*"})
	}

	return resetTokenQuery{q}
}
...
func FindResetToken(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*ResetToken, error) {
	...
	query := fmt.Sprintf(
		"select %s from `reset_tokens` where `id`=?", sel,
	)
	...
	if err = resetTokenObj.doAfterSelectHooks(ctx, exec); err != nil {
		return resetTokenObj, err
	}
...
func (o *ResetToken) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `reset_tokens` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `reset_tokens` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, resetTokenPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for reset_tokens")
	}
...
func (o *ResetToken) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE `reset_tokens` SET %s WHERE %s",
			...
			strmangle.WhereClause("`", "`", 0, resetTokenPrimaryKeyColumns),
		)
...
func (o ResetTokenSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `reset_tokens` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, resetTokenPrimaryKeyColumns, len(o)))
...
func (o *ResetToken) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `reset_tokens` WHERE `id`=?"
...
func ResetTokenExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `reset_tokens` where `id`=? limit 1)"
...
var mySQLResetTokenUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ResetToken) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(resetTokenColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLResetTokenUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`reset_tokens`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `reset_tokens` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for reset_tokens")
	}
	...
	uniqueMap, err = queries.BindMapping(resetTokenType, resetTokenMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for reset_tokens")
	}
