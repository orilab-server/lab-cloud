...
func testUserToManyDirs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, dirDBTypes, false, dirColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, dirDBTypes, false, dirColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDirs(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Dirs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Dirs = nil
	if err = a.L.LoadDirs(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Dirs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyFiles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFiles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpDirs(t *testing.T) {
	var err error
	...
	foreigners := []*Dir{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, dirDBTypes, false, strmangle.SetComplement(dirPrimaryKeyColumns, dirColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Dir{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDirs(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Dirs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Dirs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Dirs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpFiles(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `text`, `Email`: `text`, `Password`: `text`, `Grade`: `integer`, `IsTemporary`: `boolean`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
