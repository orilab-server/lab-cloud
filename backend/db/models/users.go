...
// Dirs retrieves all the dir's Dirs with an executor.
func (o *User) Dirs(mods ...qm.QueryMod) dirQuery {
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"dirs\".*"})
	}
...
func (o *User) Files(mods ...qm.QueryMod) fileQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// LoadDirs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDirs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`dirs`), qm.WhereIn(`dirs.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load dirs")
	}

	var resultSlice []*Dir
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice dirs")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on dirs")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Dirs = append(local.R.Dirs, foreign)
				if foreign.R == nil {
					foreign.R = &dirR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`files`), qm.WhereIn(`files.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// AddDirs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Dirs.
// Sets related.R.User appropriately.
func (o *User) AddDirs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Dir) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Dirs = append(o.R.Dirs, related...)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddFiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*File) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
